...
#' measure of confidence for the number of sub-populations is provided (relative
#' standard error; RSE).
#'
#' @param xVariable The fluorescence channel on the x-axis (determined by the
#'  set-up of your flow cytometer; ex. FLA-1, FITC-A)
#' @param flowDir The directory of the gated .fcs data. If NA, then a window
#'  will prompt the user to select a folder (string file path)
#' @param doublet TRUE/FALSE for having the information about doublets in the output
#'  dataset - by default FALSE (logical)
#' @param saveOutputGraph TRUE/FALSE for saving the graphs as an output of the NLS (logical)
#' @param singleThreshold threshold for classifying single populations (numerical)
#' @param usedCellsThreshold threshold for classifying multiple populations (numerical)
#' @param maxDoubletHeight The maximum height a doublet can be. If left as NA
#'  the algorithm will find a value based on the other peaks. (numerical)
#' @param subsetDs A vector of samples that the user wants to analyze if they
#'  do not want to analyse the full dataset; default value is NA (vector
...
## gaussian formula
.gaussian <- function(N, mean, sd, x) {
  return ((N/(sqrt(2*pi)*sd))*exp(-((x-mean)^2)/(2*sd^2)))
}
...
## mod-gaussian formula
.mod_gaussian <- function(sd, mean, x) {
  return ((1/(sqrt(2*pi)*sd*(x/mean)))*exp(-((x-mean)^2)/(2*(sd*(x/mean))^2)))
}
...
.popConfidenceInitial = function(flowDir, ds, xVariable, saveGraph){
  ...
  for(k in seq_len(length(flowNameDs))){
    ...
    if((!is.na(flowDataMeans$G2_1)) && (flowDataMeans$G2_1 != 0)){
      midPoint <- flowData[
        which(
          abs(
            flowData$x-mean(
